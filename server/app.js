...
var app = express();
var fs = require('fs');
var debug = require('debug')('_server:app');

// var port = (process.env.PORT_ENV && parseInt(process.env.PORT_ENV, 10)) || 6188;
// port 80 is necessary for wechat server
var port = 80;
var socketPort = 8989;
...
var wechatRouter = require('./wechat/routes/router.js').wechatRouter;
var wechatTokenServer = require('./wechat/server/token_proxy.js');
...
app.use(express.static(__dirname + '/wechat/views'));
app.use(express.static(__dirname + '/wechat/config/MP_verify_2MqdnfDI71aSkDSI.txt'));
// front static server
// Mount front API //////////////////////////////////////////////
...
var startServer = function() {
  ...
  app.listen(port);
  debug('dmtec pxiaomi socket server listening on ' + socketPort);
  debug('dmtec pxiaomi http server listening on ' + port);
};
//wechat
wechatTokenServer.start(startServer);
